...
exports.isAddressValid = function(addr){
    try { 
        console.info('addr: ' + addr);
        var decoded = base58.decode(addr);
        console.info('base58 decode: ' + decoded);
        if (decoded && decoded.length == 25){
            return true;
        } else {
            console.error('invalid address, can not decode it with base58: ' + addr);
            return false;
        }
    }
    catch(e){
        console.error('exception: ' + addr);
        return null;
    }
};
